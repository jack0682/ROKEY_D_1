...
class SetDesiredForce_Request(metaclass=Metaclass_SetDesiredForce_Request):
    ...
    def __init__(self, **kwargs):
        ...
        if 'fd' not in kwargs:
            ...
            self.fd = numpy.array(kwargs.get('fd'), dtype=numpy.float64)
            assert self.fd.shape == (6, )
        if 'dir' not in kwargs:
            self.dir = numpy.zeros(6, dtype=numpy.int8)
        else:
            self.dir = numpy.array(kwargs.get('dir'), dtype=numpy.int8)
            assert self.dir.shape == (6, )
...
class Metaclass_SetDesiredForce(type):
    ...
    def __import_type_support__(cls):
        try:
            ...
            import traceback
            logger = logging.getLogger(
                'doosan_m0609_msgs.srv.SetDesiredForce')
            logger.debug(
                'Failed to import needed modules for type support:\n' +
                traceback.format_exc())
        else:
            ...
            from doosan_m0609_msgs.srv import _set_desired_force
...
class SetDesiredForce(metaclass=Metaclass_SetDesiredForce):
    from doosan_m0609_msgs.srv._set_desired_force import SetDesiredForce_Request as Request
    from doosan_m0609_msgs.srv._set_desired_force import SetDesiredForce_Response as Response
