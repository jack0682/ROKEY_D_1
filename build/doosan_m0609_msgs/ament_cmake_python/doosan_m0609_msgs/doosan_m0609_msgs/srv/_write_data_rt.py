...
class WriteDataRt_Request(metaclass=Metaclass_WriteDataRt_Request):
    ...
    def __init__(self, **kwargs):
        ...
        if 'external_force_torque' not in kwargs:
            ...
            self.external_force_torque = numpy.array(kwargs.get('external_force_torque'), dtype=numpy.float64)
            assert self.external_force_torque.shape == (6, )
        ...
        if 'external_analog_input' not in kwargs:
            ...
            self.external_analog_input = numpy.array(kwargs.get('external_analog_input'), dtype=numpy.float64)
            assert self.external_analog_input.shape == (6, )
        if 'external_analog_output' not in kwargs:
            self.external_analog_output = numpy.zeros(6, dtype=numpy.float64)
        else:
            self.external_analog_output = numpy.array(kwargs.get('external_analog_output'), dtype=numpy.float64)
            assert self.external_analog_output.shape == (6, )
...
class Metaclass_WriteDataRt(type):
    ...
    def __import_type_support__(cls):
        try:
            ...
            import traceback
            logger = logging.getLogger(
                'doosan_m0609_msgs.srv.WriteDataRt')
            logger.debug(
                'Failed to import needed modules for type support:\n' +
                traceback.format_exc())
        else:
            ...
            from doosan_m0609_msgs.srv import _write_data_rt
...
class WriteDataRt(metaclass=Metaclass_WriteDataRt):
    from doosan_m0609_msgs.srv._write_data_rt import WriteDataRt_Request as Request
    from doosan_m0609_msgs.srv._write_data_rt import WriteDataRt_Response as Response
