    ...
    def __import_type_support__(cls):
        try:
            ...
            logger = logging.getLogger(
                'doosan_m0609_controller.msg.JointState')
            logger.debug(
...
class JointState(metaclass=Metaclass_JointState):
    ...
    def effort(self, value):
        if isinstance(value, numpy.ndarray):
            ...
            assert value.size == 6, \
                "The 'effort' numpy.ndarray() must have a size of 6"
            self._effort = value
            return
        if __debug__:
            from collections.abc import Sequence
            ...
            assert \
                ((isinstance(value, Sequence) or
                 ...
                 all(isinstance(v, float) for v in value) and
                 all(not (val < -1.7976931348623157e+308 or val > 1.7976931348623157e+308) or math.isinf(val) for val in value)), \
                "The 'effort' field must be a set or sequence with length 6 and each value of type 'float' and each double in [-.000000, .000000]"
