...
class YOLOv7InferNode(Node):
    def __init__(self):
        super().__init__('yolov7_infer_node')

        self.declare_parameter('model_path', '/yolo_experiments_ws/models/yolov7.pt')
        ...
        self.get_logger().info(f'YOLOv7 Inference Node started with model: {model_path}')

    def image_callback(self, msg):
        # 최초 시작 시 초기화
        if not hasattr(self, 'log_entries'):
            self.log_entries = []
            self.model_name = 'YOLOv7'  # 모델에 따라 바꿔줘
        ...
        if elapsed >= 60.0:
            avg_infer = self.total_infer_time / max(self.frame_count, 1)
            fps = self.frame_count / elapsed

            # 전체 요약 출력
            self.get_logger().info(f' [{self.model_name}] 평가 종료')
            self.get_logger().info(f' 총 프레임: {self.frame_count}, 평균 추론 시간: {avg_infer:.4f}s, FPS: {fps:.2f}')
