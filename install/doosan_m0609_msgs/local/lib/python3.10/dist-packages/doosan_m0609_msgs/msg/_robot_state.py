    ...
    def __import_type_support__(cls):
        try:
            ...
            logger = logging.getLogger(
                'doosan_m0609_msgs.msg.RobotState')
            logger.debug(
            ...
            from doosan_m0609_msgs.msg import ModbusState
...
class RobotState(metaclass=Metaclass_RobotState):
    ...
    _fields_and_field_types = {
        ...
        'modbus_state': 'sequence<doosan_m0609_msgs/ModbusState>',
    ...
    SLOT_TYPES = (
        ...
        rosidl_parser.definition.Array(rosidl_parser.definition.BasicType('int8'), 6),  # noqa: E501
        rosidl_parser.definition.Array(rosidl_parser.definition.BasicType('int8'), 6),  # noqa: E501
        rosidl_parser.definition.UnboundedSequence(rosidl_parser.definition.NamespacedType(['doosan_m0609_msgs', 'msg'], 'ModbusState')),  # noqa: E501
        rosidl_parser.definition.BasicType('int32'),  # noqa: E501
    ...
    def __init__(self, **kwargs):
        ...
        if 'current_posj' not in kwargs:
            ...
            self.current_posj = numpy.array(kwargs.get('current_posj'), dtype=numpy.float64)
            assert self.current_posj.shape == (6, )
        if 'current_velj' not in kwargs:
            self.current_velj = numpy.zeros(6, dtype=numpy.float64)
        else:
            self.current_velj = numpy.array(kwargs.get('current_velj'), dtype=numpy.float64)
            assert self.current_velj.shape == (6, )
        if 'joint_abs' not in kwargs:
            self.joint_abs = numpy.zeros(6, dtype=numpy.float64)
        else:
            self.joint_abs = numpy.array(kwargs.get('joint_abs'), dtype=numpy.float64)
            assert self.joint_abs.shape == (6, )
        if 'joint_err' not in kwargs:
            self.joint_err = numpy.zeros(6, dtype=numpy.float64)
        else:
            self.joint_err = numpy.array(kwargs.get('joint_err'), dtype=numpy.float64)
            assert self.joint_err.shape == (6, )
        if 'target_posj' not in kwargs:
            self.target_posj = numpy.zeros(6, dtype=numpy.float64)
        else:
            self.target_posj = numpy.array(kwargs.get('target_posj'), dtype=numpy.float64)
            assert self.target_posj.shape == (6, )
        if 'target_velj' not in kwargs:
            self.target_velj = numpy.zeros(6, dtype=numpy.float64)
        else:
            self.target_velj = numpy.array(kwargs.get('target_velj'), dtype=numpy.float64)
            assert self.target_velj.shape == (6, )
        if 'current_posx' not in kwargs:
            self.current_posx = numpy.zeros(6, dtype=numpy.float64)
        else:
            self.current_posx = numpy.array(kwargs.get('current_posx'), dtype=numpy.float64)
            assert self.current_posx.shape == (6, )
        if 'current_tool_posx' not in kwargs:
            self.current_tool_posx = numpy.zeros(6, dtype=numpy.float64)
        else:
            self.current_tool_posx = numpy.array(kwargs.get('current_tool_posx'), dtype=numpy.float64)
            assert self.current_tool_posx.shape == (6, )
        if 'current_velx' not in kwargs:
            self.current_velx = numpy.zeros(6, dtype=numpy.float64)
        else:
            self.current_velx = numpy.array(kwargs.get('current_velx'), dtype=numpy.float64)
            assert self.current_velx.shape == (6, )
        if 'task_err' not in kwargs:
            self.task_err = numpy.zeros(6, dtype=numpy.float64)
        else:
            self.task_err = numpy.array(kwargs.get('task_err'), dtype=numpy.float64)
            assert self.task_err.shape == (6, )
        if 'target_velx' not in kwargs:
            self.target_velx = numpy.zeros(6, dtype=numpy.float64)
        else:
            self.target_velx = numpy.array(kwargs.get('target_velx'), dtype=numpy.float64)
            assert self.target_velx.shape == (6, )
        if 'target_posx' not in kwargs:
            self.target_posx = numpy.zeros(6, dtype=numpy.float64)
        else:
            self.target_posx = numpy.array(kwargs.get('target_posx'), dtype=numpy.float64)
            assert self.target_posx.shape == (6, )
        if 'dynamic_tor' not in kwargs:
            self.dynamic_tor = numpy.zeros(6, dtype=numpy.float64)
        else:
            self.dynamic_tor = numpy.array(kwargs.get('dynamic_tor'), dtype=numpy.float64)
            assert self.dynamic_tor.shape == (6, )
        if 'actual_jts' not in kwargs:
            self.actual_jts = numpy.zeros(6, dtype=numpy.float64)
        else:
            self.actual_jts = numpy.array(kwargs.get('actual_jts'), dtype=numpy.float64)
            assert self.actual_jts.shape == (6, )
        if 'actual_ejt' not in kwargs:
            self.actual_ejt = numpy.zeros(6, dtype=numpy.float64)
        else:
            self.actual_ejt = numpy.array(kwargs.get('actual_ejt'), dtype=numpy.float64)
            assert self.actual_ejt.shape == (6, )
        if 'actual_ett' not in kwargs:
            self.actual_ett = numpy.zeros(6, dtype=numpy.float64)
        else:
            self.actual_ett = numpy.array(kwargs.get('actual_ett'), dtype=numpy.float64)
            assert self.actual_ett.shape == (6, )
        if 'actual_bk' not in kwargs:
            self.actual_bk = numpy.zeros(6, dtype=numpy.int8)
        else:
            self.actual_bk = numpy.array(kwargs.get('actual_bk'), dtype=numpy.int8)
            assert self.actual_bk.shape == (6, )
        if 'actual_mc' not in kwargs:
            self.actual_mc = numpy.zeros(6, dtype=numpy.float64)
        else:
            self.actual_mc = numpy.array(kwargs.get('actual_mc'), dtype=numpy.float64)
            assert self.actual_mc.shape == (6, )
        if 'actual_mt' not in kwargs:
            self.actual_mt = numpy.zeros(6, dtype=numpy.float64)
        else:
            self.actual_mt = numpy.array(kwargs.get('actual_mt'), dtype=numpy.float64)
            assert self.actual_mt.shape == (6, )
        ...
        if 'actual_bt' not in kwargs:
            ...
            self.actual_bt = numpy.array(kwargs.get('actual_bt'), dtype=numpy.int8)
            assert self.actual_bt.shape == (5, )
        ...
        if 'ctrlbox_digital_input' not in kwargs:
            ...
            self.ctrlbox_digital_input = numpy.array(kwargs.get('ctrlbox_digital_input'), dtype=numpy.int8)
            assert self.ctrlbox_digital_input.shape == (16, )
        if 'ctrlbox_digital_output' not in kwargs:
            self.ctrlbox_digital_output = numpy.zeros(16, dtype=numpy.int8)
        else:
            self.ctrlbox_digital_output = numpy.array(kwargs.get('ctrlbox_digital_output'), dtype=numpy.int8)
            assert self.ctrlbox_digital_output.shape == (16, )
        if 'flange_digital_input' not in kwargs:
            self.flange_digital_input = numpy.zeros(6, dtype=numpy.int8)
        else:
            self.flange_digital_input = numpy.array(kwargs.get('flange_digital_input'), dtype=numpy.int8)
            assert self.flange_digital_input.shape == (6, )
        if 'flange_digital_output' not in kwargs:
            self.flange_digital_output = numpy.zeros(6, dtype=numpy.int8)
        else:
            self.flange_digital_output = numpy.array(kwargs.get('flange_digital_output'), dtype=numpy.int8)
            assert self.flange_digital_output.shape == (6, )
        ...
        if 'f_actual_w2b' not in kwargs:
            ...
            self.f_actual_w2b = numpy.array(kwargs.get('f_actual_w2b'), dtype=numpy.float64)
            assert self.f_actual_w2b.shape == (6, )
        ...
        if 'f_current_vel_world' not in kwargs:
            ...
            self.f_current_vel_world = numpy.array(kwargs.get('f_current_vel_world'), dtype=numpy.float64)
            assert self.f_current_vel_world.shape == (6, )
        if 'f_world_ext_target_torque' not in kwargs:
            self.f_world_ext_target_torque = numpy.zeros(6, dtype=numpy.float64)
        else:
            self.f_world_ext_target_torque = numpy.array(kwargs.get('f_world_ext_target_torque'), dtype=numpy.float64)
            assert self.f_world_ext_target_torque.shape == (6, )
        if 'f_target_pos_world' not in kwargs:
            self.f_target_pos_world = numpy.zeros(6, dtype=numpy.float64)
        else:
            self.f_target_pos_world = numpy.array(kwargs.get('f_target_pos_world'), dtype=numpy.float64)
            assert self.f_target_pos_world.shape == (6, )
        if 'f_target_vel_world' not in kwargs:
            self.f_target_vel_world = numpy.zeros(6, dtype=numpy.float64)
        else:
            self.f_target_vel_world = numpy.array(kwargs.get('f_target_vel_world'), dtype=numpy.float64)
            assert self.f_target_vel_world.shape == (6, )
        ...
        if 'f_current_vel_user' not in kwargs:
            ...
            self.f_current_vel_user = numpy.array(kwargs.get('f_current_vel_user'), dtype=numpy.float64)
            assert self.f_current_vel_user.shape == (6, )
        if 'f_user_ext_task_torque' not in kwargs:
            self.f_user_ext_task_torque = numpy.zeros(6, dtype=numpy.float64)
        else:
            self.f_user_ext_task_torque = numpy.array(kwargs.get('f_user_ext_task_torque'), dtype=numpy.float64)
            assert self.f_user_ext_task_torque.shape == (6, )
        if 'f_target_pos_user' not in kwargs:
            self.f_target_pos_user = numpy.zeros(6, dtype=numpy.float64)
        else:
            self.f_target_pos_user = numpy.array(kwargs.get('f_target_pos_user'), dtype=numpy.float64)
            assert self.f_target_pos_user.shape == (6, )
        if 'f_target_vel_user' not in kwargs:
            self.f_target_vel_user = numpy.zeros(6, dtype=numpy.float64)
        else:
            self.f_target_vel_user = numpy.array(kwargs.get('f_target_vel_user'), dtype=numpy.float64)
            assert self.f_target_vel_user.shape == (6, )
        ...
        if 'f_actual_analog_input' not in kwargs:
            ...
            self.f_actual_analog_input = numpy.array(kwargs.get('f_actual_analog_input'), dtype=numpy.float64)
            assert self.f_actual_analog_input.shape == (6, )
        ...
        if 'i_actual_analog_input_type' not in kwargs:
            ...
            self.i_actual_analog_input_type = numpy.array(kwargs.get('i_actual_analog_input_type'), dtype=numpy.int8)
            assert self.i_actual_analog_input_type.shape == (2, )
        if 'f_target_analog_output' not in kwargs:
            self.f_target_analog_output = numpy.zeros(2, dtype=numpy.float64)
        else:
            self.f_target_analog_output = numpy.array(kwargs.get('f_target_analog_output'), dtype=numpy.float64)
            assert self.f_target_analog_output.shape == (2, )
        if 'i_target_analog_output_type' not in kwargs:
            self.i_target_analog_output_type = numpy.zeros(2, dtype=numpy.int8)
        else:
            self.i_target_analog_output_type = numpy.array(kwargs.get('i_target_analog_output_type'), dtype=numpy.int8)
            assert self.i_target_analog_output_type.shape == (2, )
        ...
        if 'i_actual_encorder_raw_data' not in kwargs:
            ...
            self.i_actual_encorder_raw_data = numpy.array(kwargs.get('i_actual_encorder_raw_data'), dtype=numpy.int8)
            assert self.i_actual_encorder_raw_data.shape == (2, )
