...
class CoordTransform_Request(metaclass=Metaclass_CoordTransform_Request):
    ...
    def __init__(self, **kwargs):
        ...
        if 'pos_in' not in kwargs:
            ...
            self.pos_in = numpy.array(kwargs.get('pos_in'), dtype=numpy.float64)
            assert self.pos_in.shape == (6, )
...
class Metaclass_CoordTransform(type):
    ...
    def __import_type_support__(cls):
        try:
            ...
            import traceback
            logger = logging.getLogger(
                'doosan_m0609_msgs.srv.CoordTransform')
            logger.debug(
                'Failed to import needed modules for type support:\n' +
                traceback.format_exc())
        else:
            ...
            from doosan_m0609_msgs.srv import _coord_transform
...
class CoordTransform(metaclass=Metaclass_CoordTransform):
    from doosan_m0609_msgs.srv._coord_transform import CoordTransform_Request as Request
    from doosan_m0609_msgs.srv._coord_transform import CoordTransform_Response as Response
