...
class EnableAlterMotion_Request(metaclass=Metaclass_EnableAlterMotion_Request):
    ...
    def __init__(self, **kwargs):
        ...
        if 'limit_dpos' not in kwargs:
            ...
            self.limit_dpos = numpy.array(kwargs.get('limit_dpos'), dtype=numpy.float64)
            assert self.limit_dpos.shape == (2, )
        if 'limit_dpos_per' not in kwargs:
            self.limit_dpos_per = numpy.zeros(2, dtype=numpy.float64)
        else:
            self.limit_dpos_per = numpy.array(kwargs.get('limit_dpos_per'), dtype=numpy.float64)
            assert self.limit_dpos_per.shape == (2, )
...
class Metaclass_EnableAlterMotion(type):
    ...
    def __import_type_support__(cls):
        try:
            ...
            import traceback
            logger = logging.getLogger(
                'doosan_m0609_msgs.srv.EnableAlterMotion')
            logger.debug(
                'Failed to import needed modules for type support:\n' +
                traceback.format_exc())
        else:
            ...
            from doosan_m0609_msgs.srv import _enable_alter_motion
...
class EnableAlterMotion(metaclass=Metaclass_EnableAlterMotion):
    from doosan_m0609_msgs.srv._enable_alter_motion import EnableAlterMotion_Request as Request
    from doosan_m0609_msgs.srv._enable_alter_motion import EnableAlterMotion_Response as Response
