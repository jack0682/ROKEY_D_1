...
# 설정
# ========================
#MODEL_PATH        = '/home/rokey/mqtt-influx/ws/turtlebot4_beep/turtlebot4_beep/best(2).pt' 
MODEL_PATH        = '/home/rokey/mqtt-influx/ws/turtlebot4_beep/turtlebot4_beep/real_best_320.pt' 
...
#     # ✅ 연한 브라운 범위: 좀 더 유연하게 (Hue: 10~35, Sat: 30~180, Val: 60~220)
#     lower_brown = np.array([10, 30, 60])
#     upper_brown = np.array([35, 180, 220])

#     # 마스크 생성
#     brown_mask = cv2.inRange(hsv_img, lower_brown, upper_brown)

#     # 🔧 Morphology로 노이즈 제거 + 경계 확장
#     kernel = np.ones((3, 3), np.uint8)
#     brown_mask = cv2.morphologyEx(brown_mask, cv2.MORPH_OPEN, kernel, iterations=1)
#     brown_mask = cv2.morphologyEx(brown_mask, cv2.MORPH_CLOSE, kernel, iterations=1)

#     # ✅ 경계 강화: 흰색으로 덮이지 않도록 살짝 확장
#     brown_mask = cv2.dilate(brown_mask, kernel, iterations=1)

#     # 배경 유지
#     brown_mask_3ch = cv2.merge([brown_mask]*3)
#     background = cv2.bitwise_and(img, brown_mask_3ch)

#     # 나머지 영역 흰색으로
#     white_bg = np.full_like(img, 255)
#     inv_mask_3ch = cv2.bitwise_not(brown_mask_3ch)
#     foreground = cv2.bitwise_and(white_bg, inv_mask_3ch)

#     result = cv2.add(background, foreground)
#     brown_pixel_count = np.count_nonzero(brown_mask)

#     return result, brown_pixel_count
...
def seg(img):
    ...
    # ✅ 3. 경계 확장 보정 (Dilate → 가장자리 여유 확보)
    correction_kernel = np.ones((5, 5), np.uint8)
    expanded_mask = cv2.dilate(brown_mask, correction_kernel, iterations=2)

    # ✅ 4. 원래 마스크와 확장 마스크의 차이 → 보정 영역
    correction_region = cv2.subtract(expanded_mask, brown_mask)

    # ✅ 5. correction_region은 무조건 흰색으로 칠함
    brown_mask_final = cv2.bitwise_and(brown_mask, brown_mask)  # 복사
    brown_mask_final[correction_region > 0] = 0  # 경계 제거

    # 6. 이미지 마스킹
    brown_mask_3ch = cv2.merge([brown_mask_final]*3)
    background = cv2.bitwise_and(img, brown_mask_3ch)
    ...
    # 픽셀 수 정보
    brown_pixel_count = np.count_nonzero(brown_mask_final)
            ...
            with self.lock:
                ...
                for r in results:
                    for box in r.boxes:
                        ...
                        y_start = max(detect_cy - half_w, 0)
                        y_end   = min(detect_cy + half_w + 1, depth_mm.shape[0])
                        x_start = max(detect_cx - half_w, 0)
                        x_end   = min(detect_cx + half_w + 1, depth_mm.shape[1])

                        depth_patch = depth_mm[y_start:y_end, x_start:x_end]

                        # 0 또는 NaN 제거 후 평균
                        valid_depths = depth_patch[(depth_patch > 0) & (~np.isnan(depth_patch))]

                        if valid_depths.size == 0:
                            self.get_logger().warn(f"No valid depth around ({detect_cx}, {detect_cy})")
                            continue

                        distance_m = (np.mean(valid_depths) / 1000.0) 
                        ...
                        if 0 <= detect_cx < depth_mm.shape[1] and 0 <= detect_cy < depth_mm.shape[0]:
                            ...
                            distance_m = distance_mm / 1000.0 - 0.3
